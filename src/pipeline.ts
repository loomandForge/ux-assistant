...
import {
  ...
  ReviewDetailPayload,
  buildStrategicArtifacts
} from './detail.js';
import { STRATEGIC_CONTRACT_VERSION } from './contract.js';
import { ReviewStage, STAGE_MESSAGE } from './stages.js';
import { parseDesignSystemMode, type DesignSystemConfig } from './design-system.js';
...
export interface ReviewResult {
  ...
  scores: Array<{ parameter: string; alignmentPct: number; score: number }>;
  topIssues: Array<{ title: string; severity: string; parameter: string }>;
}
...
const shouldAutoSaveMarkdown = (): boolean => {
...
const resolveReportOutputDir = (): string => {
...
const buildSourceSlug = (source: string): string => {
  try {
...
const persistMarkdownFile = (runId: number, source: string, markdown: string): string | undefined => {
...
const runScoringAndPersist = async (
  inputType: ReviewInputType,
  source: string,
  ...
  let designSystemEvidence: DesignSystemEvidence | undefined;
  if (designSystemConfig?.mode !== 'none' && designSystemConfig?.enableExternalMcp) {
    try {
      ...
      designSystemEvidence = await buildDesignSystemEvidence(
        bundle.figmaUrl ?? source,
        undefined,
        debug
      );
      ...
      storage.updateDesignSystemFindings(
        runId,
        designSystemConfig.mode,
        bundle.designSystemFindings
      );
    } catch (error) {
      storage.addToolCall(
        runId,
        'query_design_system_mcp',
        'error',
        {
          mode: designSystemConfig.mode
        },
  ...
  setStage(storage, runId, 'scoring', onProgress);
  const scoringResult: DeterministicReviewResult = runDeterministicScoring(bundle);

  // Extract actual design data from tool call results for LLM enrichment
  const sourceTypeMap: Record<ReviewInputType, 'figma' | 'web' | 'html' | 'image'> = {
    figma_url: 'figma',
    web_url: 'web',
    html_snippet: 'html',
    image_path: 'image'
  ...
  const run = storage.getRun(runId) as
    | {
        created_at?: string;
        status?: string;
        stage?: string;
  ...
  const detail: ReviewDetailPayload = buildReviewDetailPayload({
    runId,
    source,
...
export async function reviewInput(
  ...
  if (
    brief.summary.hasProblemStatement ||
