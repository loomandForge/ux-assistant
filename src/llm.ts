...
import { ExtractedDesignData, formatDesignDataForPrompt } from './design-data-extract.js';
...
export interface ParameterReviewDetail {
  commentary: string;
  strengths: string[];
  issueDetails: Array<{ problem: string; impact: string; fix: string }>;
}
...
export interface PerspectiveContext {
  ...
  memoryContext?: {
    session?: Array<{ memoryKey: string; entryType: string; content: unknown; tags: string[] }>;
    user?: Array<{ memoryKey: string; entryType: string; content: unknown; tags: string[] }>;
    project?: Array<{ memoryKey: string; entryType: string; content: unknown; tags: string[] }>;
  };
  prdText?: string;
  ...
  previousReview?: { overallAlignmentPct: number; scores: Array<{ parameter: string; alignmentPct: number }> };
}
...
interface Provider {
  ...
  callMarkdown(systemPrompt: string, userPrompt: string): Promise<string>;
}

const PARAMETER_ORDER: ReviewParameterKey[] = [
...
const parseJsonOrThrow = <T>(value: string): T => {
  try {
...
const retry = async <T>(operation: () => Promise<T>, attempts: number): Promise<T> => {
  ...
  return {
    ...
    async callJson<T>(prompt: string): Promise<T> {
      const response = await withTimeout(
        signal =>
          fetch(`${options.baseUrl.replace(/\/$/, '')}/chat/completions`, {
            method: 'POST',
            headers: baseHeaders,
            body: JSON.stringify({
              model: options.model,
              ...
              messages: [
                {
                  role: 'system',
                  content:
                    'You are a UX review assistant. Always return strict JSON with no markdown fences.'
                },
      ...
      return parseJsonOrThrow<T>(content);
    },
    async callMarkdown(systemPrompt: string, userPrompt: string): Promise<string> {
      const response = await withTimeout(
        signal =>
          fetch(`${options.baseUrl.replace(/\/$/, '')}/chat/completions`, {
            method: 'POST',
...
let cachedCopilotToken: CopilotApiToken | null = null;

const readCopilotOAuthToken = (): string | undefined => {
...
const obtainCopilotApiToken = async (oauthToken: string): Promise<CopilotApiToken | null> => {
...
const getProviderChain = (): Provider[] => {
...
const toParameterLabel = (parameter: ReviewParameterKey): string => parameter.replace(/_/g, ' ');

const buildSummaryPrompt = (scoring: DeterministicReviewResult, designData?: ExtractedDesignData): string => {
  const parts = [
  ...
  parts.push(
    `Overall alignment: ${scoring.overallAlignmentPct}%`,
    `Scores: ${JSON.stringify(scoring.scores)}`,
    `High-priority issues: ${JSON.stringify(
      scoring.issues
        .filter(issue => issue.severity === 'critical' || issue.severity === 'high')
        .slice(0, 10)
    )}`,
    `All evidence: ${JSON.stringify(scoring.evidence.slice(0, 15))}`
  );

  return parts.join('\n');
};

const buildParameterPrompt = (
...
const getDesignDataForParameter = (parameter: ReviewParameterKey, data: ExtractedDesignData): string | null => {
  const sections: string[] = [];

  switch (parameter) {
    case 'user_flow_interaction':
      if (data.frameNames.length > 0) sections.push(`Frames/Screens: ${data.frameNames.slice(0, 8).join(', ')}`);
      if (data.interactions.length > 0) sections.push(`Interactions: ${data.interactions.slice(0, 5).join('; ')}`);
      if (data.textContent.length > 0) sections.push(`CTA/Action text: ${data.textContent.filter(t => t.length < 30).slice(0, 8).map(t => `"${t}"`).join(', ')}`);
      break;
    case 'visual_hierarchy_layout':
      if (data.layout.length > 0) sections.push(`Layout: ${data.layout.slice(0, 6).join('; ')}`);
      if (data.typography.length > 0) sections.push(`Typography: ${data.typography.slice(0, 6).join(', ')}`);
      if (data.colors.length > 0) sections.push(`Colors: ${data.colors.slice(0, 6).join(', ')}`);
      break;
    case 'design_system_consistency':
      if (data.components.length > 0) sections.push(`Components used: ${data.components.slice(0, 12).join(', ')}`);
      if (data.tokens.length > 0) sections.push(`Tokens: ${data.tokens.slice(0, 8).join(', ')}`);
      break;
    case 'accessibility_wcag':
      if (data.colors.length > 0) sections.push(`Colors (check contrast): ${data.colors.slice(0, 6).join(', ')}`);
      if (data.typography.length > 0) sections.push(`Font sizes (check readability): ${data.typography.slice(0, 4).join(', ')}`);
      if (data.textContent.length > 0) sections.push(`Labels (check clarity): ${data.textContent.slice(0, 6).map(t => `"${t}"`).join(', ')}`);
      break;
    case 'content_information_architecture':
      if (data.textContent.length > 0) sections.push(`Content/Labels: ${data.textContent.slice(0, 10).map(t => `"${t}"`).join(', ')}`);
      if (data.frameNames.length > 0) sections.push(`Sections: ${data.frameNames.slice(0, 6).join(', ')}`);
      if (data.hierarchy.length > 0) sections.push(`Hierarchy: ${data.hierarchy.slice(0, 8).join(' > ')}`);
      break;
    case 'technical_feasibility':
      if (data.components.length > 0) sections.push(`Components: ${data.components.slice(0, 8).join(', ')}`);
      if (data.layout.length > 0) sections.push(`Layout complexity: ${data.layout.slice(0, 4).join('; ')}`);
      if (data.interactions.length > 0) sections.push(`Interactions to implement: ${data.interactions.slice(0, 4).join('; ')}`);
      break;
    case 'brand_design_quality':
      if (data.colors.length > 0) sections.push(`Brand colors: ${data.colors.slice(0, 6).join(', ')}`);
      if (data.typography.length > 0) sections.push(`Typography: ${data.typography.slice(0, 4).join(', ')}`);
      if (data.tokens.length > 0) sections.push(`Brand tokens: ${data.tokens.filter(t => t.toLowerCase().includes('brand') || t.toLowerCase().includes('color')).slice(0, 4).join(', ')}`);
      break;
  }
...
const deterministicNarrativeFallback = (
  scoring: DeterministicReviewResult,
  durationMs: number
): LlmNarrative => {
  const byParameter = Object.fromEntries(
    PARAMETER_ORDER.map(parameter => {
      ...
      const highestSeverity =
        scoring.issues.find(item => item.parameter === parameter)?.severity ?? 'low';
  ...
  ) as Record<ReviewParameterKey, string>;

  const reviewsByParameter = Object.fromEntries(
    PARAMETER_ORDER.map(parameter => {
      const score = scoring.scores.find(item => item.parameter === parameter);
      const paramIssues = scoring.issues.filter(item => item.parameter === parameter);
      const review: ParameterReviewDetail = {
  ...
  ) as Record<ReviewParameterKey, ParameterReviewDetail>;

  return {
    executiveSummary:
      `Deterministic review completed with ${scoring.overallAlignmentPct}% overall alignment. ` +
  ...
  for (let i = 0; i < providers.length; i += 1) {
    ...
    try {
      ...
      const summary = await retry(
        () =>
          provider.callJson<{ executiveSummary: string; topRisks: string[] }>(
            buildSummaryPrompt(scoring, designData)
          ),
        DEFAULT_RETRIES
      );

      const commentaryPairs = await Promise.all(
        PARAMETER_ORDER.map(async parameter => {
          const payload = await retry(
            () =>
              provider.callJson<{
                ...
                strengths?: string[];
                issueDetails?: Array<{ problem: string; impact: string; fix: string }>;
              }>(buildParameterPrompt(parameter, scoring, designData)),
            DEFAULT_RETRIES
          );
          const review: ParameterReviewDetail = {
          ...
          return [parameter, review] as const;
        })
      );

      const parameterCommentary = Object.fromEntries(
        commentaryPairs.map(([p, r]) => [p, r.commentary])
      ) as Record<ReviewParameterKey, string>;

      const parameterReviews = Object.fromEntries(commentaryPairs) as Record<
        ReviewParameterKey,
        ParameterReviewDetail
      >;
...
const PERSPECTIVE_SYSTEM_PROMPTS: Record<PerspectiveMode, string> = {
...
const buildPerspectiveUserPrompt = (ctx: PerspectiveContext): string => {
  const parts: string[] = [];
  ...
  if (ctx.problemStatement || ctx.prdText) {
    ...
    if (ctx.problemStatement) parts.push(`Problem Statement: ${ctx.problemStatement}`);
    if (ctx.proposedSolution) parts.push(`Proposed Solution: ${ctx.proposedSolution}`);
    if (ctx.requirements && ctx.requirements.length > 0) {
      parts.push('Requirements:');
      for (const req of ctx.requirements) parts.push(`- ${req}`);
    }
    if (ctx.prdText) {
      parts.push('');
  ...
  if (ctx.audience || ctx.businessGoal) {
    ...
    if (ctx.businessGoal) parts.push(`Business Goal: ${ctx.businessGoal}`);
    parts.push('');
  }

  if (ctx.knowledgeContext) {
    parts.push('## Relevant Knowledge Context');
    if (ctx.knowledgeContext.userId) parts.push(`User ID: ${ctx.knowledgeContext.userId}`);
    if (ctx.knowledgeContext.projectId !== undefined) parts.push(`Project ID: ${ctx.knowledgeContext.projectId}`);
    if (ctx.knowledgeContext.sessionId) parts.push(`Session ID: ${ctx.knowledgeContext.sessionId}`);
    if (Array.isArray(ctx.knowledgeContext.items) && ctx.knowledgeContext.items.length > 0) {
      ...
      for (const item of ctx.knowledgeContext.items.slice(0, 12)) {
        ...
        parts.push(
          `- [${item.scope}/${item.priority}/${item.confidence}] ${item.category}: ${item.summary} (${item.knowledgeKey})${tags}${item.source ? ` | source=${item.source}` : ''}`
        );
      }
    }
    if (Array.isArray(ctx.knowledgeContext.relationships) && ctx.knowledgeContext.relationships.length > 0) {
      ...
      for (const relationship of ctx.knowledgeContext.relationships.slice(0, 12)) {
        parts.push(
          `- ${relationship.fromKnowledgeKey} --${relationship.relationshipType}--> ${relationship.toKnowledgeKey}${relationship.note ? ` | ${relationship.note}` : ''}`
        );
      }
    }
    parts.push('');
  ...
  if (ctx.memoryContext) {
    ...
    const pushMemoryBlock = (
      label: string,
      entries?: Array<{ memoryKey: string; entryType: string; content: unknown; tags: string[] }>
    ) => {
      ...
      for (const entry of entries.slice(0, 8)) {
        ...
        parts.push(
          `- ${entry.memoryKey} [${entry.entryType}]${entry.tags.length > 0 ? ` tags=${entry.tags.join(', ')}` : ''}: ${content.slice(0, 500)}`
        );
      }
    };
  ...
  if (ctx.designDecisions && ctx.designDecisions.length > 0) {
    ...
    for (const decision of ctx.designDecisions) parts.push(`- ${decision}`);
    parts.push('');
  }
  if (ctx.constraints && ctx.constraints.length > 0) {
    parts.push('## Design Constraints');
    for (const constraint of ctx.constraints) parts.push(`- ${constraint}`);
    parts.push('');
  }
  if (ctx.alternativesConsidered && ctx.alternativesConsidered.length > 0) {
    parts.push('## Alternatives Considered & Rejected');
    for (const alt of ctx.alternativesConsidered) parts.push(`- ${alt}`);
    parts.push('');
  }
  if (ctx.userResearch) {
  ...
  if (ctx.previousReview) {
    ...
    if (ctx.previousReview.scores) {
      ...
      for (const prev of ctx.previousReview.scores) {
        const current = (ctx.detail?.sections ?? []).find((s: any) => s.parameter === prev.parameter);
        if (current) {
          const d = current.alignmentPct - prev.alignmentPct;
          if (d !== 0) parts.push(`- ${prev.parameter.replace(/_/g, ' ')}: ${prev.alignmentPct}% → ${current.alignmentPct}% (${d >= 0 ? '+' : ''}${d}%)`);
        }
      }
    }
    parts.push('');
  ...
  if (ctx.detail) {
    ...
    if (Array.isArray(ctx.detail.sections)) {
      ...
      for (const section of ctx.detail.sections) {
        ...
        if (section.summary) parts.push(`  Summary: ${section.summary}`);
        ...
        if (Array.isArray(section.issues)) {
          for (const issue of section.issues.slice(0, 3)) {
            ...
            if (issue.recommendation) parts.push(`    Recommendation: ${issue.recommendation}`);
          }
        }
      }
    }
    ...
    if (Array.isArray(ctx.detail.topRisks) && ctx.detail.topRisks.length > 0) {
      ...
      for (const risk of ctx.detail.topRisks) parts.push(`- ${risk}`);
    }
  }
  ...
  if (ctx.strategicArtifacts) {
    ...
    if (Array.isArray(sa.challengePrompts) && sa.challengePrompts.length > 0) {
      ...
      for (const p of sa.challengePrompts) parts.push(`- ${p}`);
    }
    if (Array.isArray(sa.flowIaHints) && sa.flowIaHints.length > 0) {
      parts.push('');
      parts.push('### Flow & IA Hints');
      for (const h of sa.flowIaHints) parts.push(`- ${h}`);
    }
    if (sa.flowIaStructure) {
      parts.push('');
      parts.push('### Flow Structure');
      if (Array.isArray(sa.flowIaStructure.nodes)) {
        parts.push('Nodes: ' + sa.flowIaStructure.nodes.map((n: any) => `${n.id}[${n.kind}]`).join(', '));
      }
      if (Array.isArray(sa.flowIaStructure.edges)) {
        parts.push('Edges: ' + sa.flowIaStructure.edges.map((e: any) => `${e.from}->${e.to}: ${e.label}`).join('; '));
      }
      if (Array.isArray(sa.flowIaStructure.designHints)) {
        for (const h of sa.flowIaStructure.designHints) parts.push(`- Hint: ${h}`);
      }
    }
    ...
    if (sa.persuasionPack) {
      ...
      if (sa.persuasionPack.positioning) parts.push(`Positioning: ${sa.persuasionPack.positioning}`);
      if (Array.isArray(sa.persuasionPack.valuePoints)) {
        for (const v of sa.persuasionPack.valuePoints) parts.push(`- Value: ${v}`);
      }
      if (Array.isArray(sa.persuasionPack.objectionHandlers)) {
        for (const o of sa.persuasionPack.objectionHandlers) parts.push(`- Objection: ${o}`);
      }
    }
    if (sa.improvementPack) {
      ...
      if (Array.isArray(sa.improvementPack.priorityFixes)) {
        for (const f of sa.improvementPack.priorityFixes) parts.push(`- Fix: ${f}`);
      }
      if (Array.isArray(sa.improvementPack.edgeCaseChecks)) {
        for (const c of sa.improvementPack.edgeCaseChecks) parts.push(`- Check: ${c}`);
      }
    }
  }
...
export const generatePerspectiveReport = async (
  ctx: PerspectiveContext,
  debug = false
): Promise<PerspectiveReportResult> => {
  const startedAt = Date.now();
  const providers = await getProviderChainWithCopilot();
  ...
  for (let i = 0; i < providers.length; i += 1) {
    ...
    try {
      ...
      const markdown = await retry(
        () => provider.callMarkdown(systemPrompt, userPrompt),
        DEFAULT_RETRIES
      );
