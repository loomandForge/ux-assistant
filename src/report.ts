...
import { LlmNarrative, ParameterReviewDetail } from './llm.js';
...
interface StrategicArtifacts {
  ...
  flowIaHints: string[];
  flowIaStructure?: {
    nodes: Array<{ id: string; label: string; kind: 'entry' | 'task' | 'decision' | 'outcome' }>;
...
export function buildMarkdownReport(
  ...
  strategicArtifacts?: StrategicArtifacts
): string {
  const lines: string[] = [];
  ...
  if (narrative) {
    lines.push(
      `**Narrative Provider:** ${narrative.provider.provider} (${narrative.provider.model})`
    );
  ...
  if (result.strategicBranch) {
    const branchLabel =
      result.strategicBranch.branch === 'improvement'
        ? 'Improve Solution'
    ...
    lines.push(
      `- **Requirement Traceability:** ${result.strategicBranch.requirementTraceabilityPct}%`
    );
  ...
  const designSystemIssue = result.issues.find(
    issue =>
      issue.parameter === 'design_system_consistency' &&
      issue.title.toLowerCase().includes('design system checks failed')
  );
  const designSystemEvidence = result.evidence.find(
    item =>
      item.parameter === 'design_system_consistency' &&
  ...
  for (const s of result.scores) {
    ...
    const review: ParameterReviewDetail | undefined = narrative?.parameterReviews?.[s.parameter];
  ...
  if (highIssues.length > 0 || mediumIssues.length > 0) {
    ...
    const parameterIssueCounts = new Map<string, number>();
