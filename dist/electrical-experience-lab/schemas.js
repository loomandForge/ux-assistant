import { z } from 'zod';
import { validationConfidenceSchema, validationSeveritySchema, validationStatusSchema } from '../contract.js';
export const AX_STAGE_ORDER = ['SEE', 'UNDERSTAND', 'PLAN', 'ACT', 'VERIFY'];
...
export const scenario001AutonomyModeSchema = z.enum(SCENARIO_001_ALLOWED_AUTONOMY_MODES);
...
export const energyPeriodSchema = z
    ...
    const equipmentTotal = period.equipmentEnergyKwh.hvac +
...
const scenario001AllowedModeSet = new Set(SCENARIO_001_ALLOWED_AUTONOMY_MODES);
export const scenario001AxRecommendationSchema = axRecommendationSchema.superRefine((recommendation, context) => {
    ...
    for (const mode of modesToValidate) {
        if (!scenario001AllowedModeSet.has(mode)) {
            context.addIssue({
                ...
                message: 'Scenario 001 is restricted to Assist, Investigate, and Recommend autonomy.'
            });
            break;
        }
    }
