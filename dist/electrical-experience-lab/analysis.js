import { calculateEquipmentContribution, calculateOperatingHoursDelta, calculatePercentageChange } from './calculations.js';
import { electricalAnalysisSchema } from './schemas.js';
export const analyzeElectricalScenario = (scenario) => {
    ...
    const equipmentContributions = equipment.map(item => calculateEquipmentContribution({
    ...
    return electricalAnalysisSchema.parse({
        ...
        facts: [
            {
                ...
                statement: `Total energy was ${scenario.baseline.totalEnergyKwh} kWh in the baseline and ` +
            ...
            ...equipment.map(item => ({
                ...
                statement: `${item.label} energy was ${item.baselineKwh} kWh in the baseline and ` +
                    `${item.currentKwh} kWh in the current period.`,
                sourcePath: item.sourcePath
            })),
            {
                id: 'fact.operating-hours.hvac',
                statement: `HVAC operated ${scenario.baseline.hvacOperatingHoursPerDay} hours per day in the ` +
        ...
        hypotheses: [
            {
                id: hypothesisId,
                statement: 'Longer HVAC operating time may be contributing to the measured HVAC energy increase.',
                supportedBy: [
                    'calc.equipment-contribution.hvac',
                    operatingHoursDelta.id
                ],
                confidence: 'medium',
                limitation: 'The measurements show correlation, not why the operating schedule changed or whether it is the sole cause.'
            }
        ],
        ...
        recommendedChecks: [
            {
                ...
                rationale: 'Confirm when the extra operating hours occurred before considering any schedule change.',
                evidenceRefs: [
                    'calc.equipment-contribution.hvac',
                    operatingHoursDelta.id,
                    hypothesisId
        ...
        confidence: {
            score: 82,
            level: 'high',
            rationale: 'The energy and operating-hours comparisons are deterministic; the schedule-change reason remains unknown.'
        }
    });
};
