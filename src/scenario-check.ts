...
const baseResult = (): DeterministicReviewResult => ({
  scores: [
    {
...
const verifyWeakScenario = (): void => {
  const result = baseResult();
  const base = result.strategicBranch;
  if (!base) throw new Error('baseResult must provide strategicBranch');
  ...
  assert(
    (artifacts.edgeCaseFindings?.length ?? 0) > 0,
    'Weak scenario must include edgeCaseFindings'
  );
};

const verifyStrongScenario = (): void => {
  const result = baseResult();
  const base = result.strategicBranch;
  if (!base) throw new Error('baseResult must provide strategicBranch');
  result.strategicBranch = {
  ...
  assert(
    (artifacts.flowIaStructure?.nodes.length ?? 0) >= 5,
    'Strong scenario must include structured flow nodes'
  );
  assert(
    (artifacts.flowIaStructure?.edges.length ?? 0) >= 5,
    'Strong scenario must include structured flow edges'
  );
  assert(
    (artifacts.flowIaStructure?.scenarios.length ?? 0) >= 3,
    'Strong scenario must include flow scenarios'
  );
};

const verifyDualScenario = (): void => {
  const result = baseResult();
  const base = result.strategicBranch;
  if (!base) throw new Error('baseResult must provide strategicBranch');
  result.strategicBranch = {
  ...
  const scenarioNames = new Set(
    (artifacts.flowIaStructure?.scenarios ?? []).map(item => item.name)
  );
