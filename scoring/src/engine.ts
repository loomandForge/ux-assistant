import { ScoringBundle } from './types.js';
import {
...
/* ──────────────────────────────────────────────────────────────────
 * Percentage-based scoring engine (Pure Evidence Model)
 *
 * Design goals:
 *   • Score reflects ONLY observable evidence from MCP tools
 *   • No base weights → full 0-100 range utilization
 *   • Missing documentation = lower score (incentivizes compliance)
 *   • Tools that succeed contribute proportionally to the score
 *   • Unknown gaps (high-impact) get proportional penalties
 *
 * Formula per parameter:
 *   depWeight      = 100 / deps.length
 *   alignment      = Σ(hit deps × depWeight) − Σ(unknownPenalty)
 *   clamped        = clamp(alignment, 0, 100)
 *
 * Overall = average of all 7 parameter alignment percentages.
 * ────────────────────────────────────────────────────────────────── */
...
interface DesignSystemAugmentResult {
  bonusPct: number;
  evidence: ReviewEvidence[];
  issues: ReviewIssue[];
}
...
const UNKNOWN_PENALTY: Record<string, number> = {
  high: 20,
  medium: 12,
  low: 8
};
...
const hasToolSuccess = (bundle: ScoringBundle, toolName: string): boolean => {
  return bundle.toolCalls.some(tc => tc.toolName === toolName && tc.status === 'success');
};

const parameterSummary = (label: string, pct: number): string => {
  if (pct >= 85) return `${label} is well-documented with comprehensive evidence.`;
  if (pct >= 70) return `${label} has good coverage but missing some key documentation.`;
  if (pct >= 50) return `${label} has moderate coverage with notable gaps.`;
  if (pct >= 30) return `${label} has limited documentation and needs significant work.`;
...
const evaluateParameter = (
  ...
  dependencies: { tool: string; label: string }[],
  unknownChecks: Array<{
    label: string;
...
const evaluateDesignSystemAugment = (bundle: ScoringBundle): DesignSystemAugmentResult => {
 ...
 */
export const runDeterministicScoring = (bundle: ScoringBundle): DeterministicReviewResult => {
  ...
  for (const def of DEFAULT_DEFINITIONS) {
    ...
    const designSystemAugment =
      def.parameter === 'design_system_consistency'
        ? evaluateDesignSystemAugment(bundle)
        : { bonusPct: 0, evidence: [] as ReviewEvidence[], issues: [] as ReviewIssue[] };
  ...
  const overallAlignmentPct = Math.round(
    scores.reduce((sum, s) => sum + s.alignmentPct, 0) / scores.length
  );
