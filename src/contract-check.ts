...
import {
  ...
  strategicArtifactsSchema,
  type StrategicArtifactsContract
} from './contract.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);

const loadFixture = (name: string): StrategicArtifactsContract => {
...
const main = (): void => {
  ...
  assert(
    (weak.edgeCaseFindings?.length ?? 0) > 0,
    'Weak fixture must include tagged edgeCaseFindings'
  );
  assert(
    (strong.flowIaStructure?.nodes.length ?? 0) >= 5,
    'Strong fixture must include rich flowIaStructure nodes'
  );
  assert(
    (strong.flowIaStructure?.edges.length ?? 0) >= 5,
    'Strong fixture must include rich flowIaStructure edges'
  );
  assert(
    (strong.flowIaStructure?.scenarios.length ?? 0) >= 3,
    'Strong fixture must include scenario diversity'
  );
