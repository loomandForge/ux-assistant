...
import { severityFromPriority } from './validators/types.js';
const severityRank = {
    critical: 0,
    high: 1,
    medium: 2,
...
export const scoreFromStatuses = (statuses) => {
...
export const evaluateRuleFinding = (rule, outputType, outputContent) => {
    ...
    return {
        ...
        severity,
        confidence: 'low',
        evidence: `No deterministic validator is available yet for validatorType=${rule.validatorType} and outputType=${outputType}.`,
        recommendation: 'Use multimodal or domain-specific validator in the next pass to verify this rule.',
        correctionPrompt: 'Re-check this output with a validator capable of evaluating this rule type and apply corrections if non-compliant.'
    };
};
export const buildCorrectionPrompt = ({ targetTool, overallCompliance, findings, maxItems }) => {
