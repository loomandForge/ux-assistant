...
import {
  ...
  StrategicArtifactsContract,
  validateStrategicArtifacts
} from './contract.js';
...
export interface DetailSection {
  ...
  summary: string;
  narrative?: string;
  evidence: Array<{ label: string; detail: string; confidence: string }>;
  issues: Array<{ severity: string; title: string; evidence: string; recommendation: string }>;
}
...
const toBadge = (alignmentPct: number): string => {
  if (alignmentPct >= 85) return 'excellent';
  if (alignmentPct >= 70) return 'good';
  if (alignmentPct >= 50) return 'warning';
  return 'at-risk';
};

const sectionSeverity = (severities: string[]): 'critical' | 'high' | 'medium' | 'low' => {
  if (severities.length === 0) {
    return 'low';
  }

  return severities.reduce<'critical' | 'high' | 'medium' | 'low'>((current, next) => {
    return severityRank[next] > severityRank[current]
      ? (next as 'critical' | 'high' | 'medium' | 'low')
      : current;
  }, 'low');
};

const takeFirstSentence = (value: string | undefined, fallback: string): string => {
  if (!value) return fallback;
  const match = value.match(/[^.!?]+[.!?]/);
  return (match?.[0] ?? value).trim();
};

const toHumanParameter = (parameter: string): string => parameter.replace(/_/g, ' ');

const parameterLabel = (parameter: string): string =>
  toHumanParameter(parameter)
    .split(' ')
...
const severityRankValue = (severity: string): number => {
...
const parameterChallengeNudges: Record<string, string> = {
  user_flow_interaction:
    'Where do users hesitate, and what measurable signal confirms flow confidence improves?',
  visual_hierarchy_layout:
    'Which visual cue drives first action, and how will you validate faster comprehension?',
  design_system_consistency:
    'Which deviations are intentional, and where is the approval/evidence for each exception?',
  accessibility_wcag:
    'Which WCAG criteria are still unproven, and what explicit test evidence will close the gap?',
  content_information_architecture:
    'What proves labels/grouping reduce navigation mistakes for first-time users?',
  technical_feasibility:
    'What implementation risks could block delivery, and what pre-validation reduces this risk?',
  brand_design_quality:
    'Which brand cues are strongest, and how will partners perceive distinct value at a glance?'
};

const parameterEdgeCaseChecks: Record<string, string[]> = {
  user_flow_interaction: [
...
const edgeCaseTagByParameter: Record<
  string,
  'failure-scenario' | 'rare-user-path' | 'operational-exception'
> = {
...
export const buildStrategicArtifacts = (
  ...
  const highToCritical = orderedIssues.filter(
  ...
  const challengePrompts = (highToCritical.length > 0 ? highToCritical : orderedIssues)
    ...
    .map(issue => {
      const nudge =
        parameterChallengeNudges[issue.parameter] ??
  ...
  weakestScores.forEach(score => {
    const nudge =
      parameterChallengeNudges[score.parameter] ??
  ...
  if (dedupedChallengePrompts.length === 0) {
    dedupedChallengePrompts.push(
      'What is the strongest evidence that this solution improves user outcomes over the baseline?'
    );
  }
  ...
  const flowIaHints = scoring.issues
    .filter(
      issue =>
        issue.parameter === 'user_flow_interaction' ||
  ...
  if (flowIaHints.length === 0) {
    flowIaHints.push(
      'Validate the happy path, failure path, and recovery path for the primary user journey.'
    );
  ...
  const flowIaStructure = {
    nodes: [
      { id: 'entry', label: 'Entry point', kind: 'entry' as const },
      { id: 'intake', label: 'Context intake', kind: 'task' as const },
      { id: 'task-primary', label: parameterLabel(primaryFlowNode), kind: 'task' as const },
      { id: 'decision', label: 'Decision / validation gate', kind: 'decision' as const },
      { id: 'recovery', label: 'Recovery branch', kind: 'task' as const },
      { id: 'task-secondary', label: parameterLabel(secondaryFlowNode), kind: 'task' as const },
      { id: 'validation', label: 'Final validation', kind: 'decision' as const },
      { id: 'outcome', label: 'Outcome confirmation', kind: 'outcome' as const }
    ],
    edges: [
    ...
    scenarios: [
      {
        ...
        riskTag: 'failure-scenario' as const
      },
      {
        name: 'recovery-path',
        pathNodeIds: [
      ...
      },
      {
        ...
        riskTag: 'rare-user-path' as const
      }
    ]
  };
  ...
  const objectionHandlers = highToCritical
    ...
    .map(
      issue =>
        `Objection in ${toHumanParameter(issue.parameter)}: ${issue.title}. Response: ${issue.recommendation}`
    );
  ...
  const priorityFixes = (highToCritical.length > 0 ? highToCritical : orderedIssues)
    ...
    .map(
      issue =>
        `${toTitleCase(toHumanParameter(issue.parameter))} - ${issue.title}: ${issue.recommendation}`
    );

  const parameterSpecificChecks = weakestScores.flatMap(
    score => parameterEdgeCaseChecks[score.parameter] ?? []
  );
  ...
  const executiveLead = takeFirstSentence(
    narrative?.executiveSummary,
    'This solution shows potential, but confidence depends on closing observable UX and traceability gaps.'
  );

  const persuasionPack =
    branch === 'persuasion' || branch === 'dual'
      ? {
          positioning: executiveLead,
          convincePartners:
            'Lead with measurable alignment strengths and explicit mitigation plans for the remaining risks.',
  ...
  const improvementPack =
    branch === 'improvement' || branch === 'dual'
      ? {
          priorityFixes:
...
export const buildReviewDetailPayload = (options: {
  ...
  const sections: DetailSection[] = options.scoring.scores.map(score => {
  ...
  const scoreIndicators: UiScoreIndicator[] = sections.map(section => ({
