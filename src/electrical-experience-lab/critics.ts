...
import {
  criticCheckSchema,
  criticResultSchema,
  criticSuiteResultSchema,
  SCENARIO_001_ALLOWED_AUTONOMY_MODES,
  type AxRecommendation,
  type CriticCheck,
  type CriticCheckId,
  type CriticResult,
  type CriticSuiteResult,
...
const buildResult = (
  criticId: CriticResult['criticId'],
  checks: CriticCheck[]
): CriticResult => {
...
const collectAnalysisEvidenceIds = (analysis: ElectricalAnalysis): Set<string> => {
  return new Set([
...
export const runElectricalCritic = (
  ...
  for (const claim of claims) {
    ...
    if (missingRefs.length > 0) {
      unsupportedFailures.push(
        `Claim "${claim.text}" has unsupported references: ${missingRefs.join(', ')}.`
      );
    }
  }
  ...
  for (const hypothesis of analysis.hypotheses) {
    ...
    if (missingRefs.length > 0) {
      unsupportedFailures.push(
        `Hypothesis ${hypothesis.id} has missing support: ${missingRefs.join(', ')}.`
      );
    }
  }
  ...
  for (const check of analysis.recommendedChecks) {
    ...
    if (missingEvidence.length > 0 || missingUnknowns.length > 0) {
      unsupportedFailures.push(
        `Recommended check ${check.id} has unresolved references: ${[
          ...missingEvidence,
          ...missingUnknowns
        ].join(', ')}.`
      );
  ...
  for (const action of actions) {
    ...
    if (missingRefs.length > 0) {
      unsupportedFailures.push(
        `Action ${action.id} has unsupported references: ${missingRefs.join(', ')}.`
      );
    }
  }
  ...
  const unsupportedClaims = buildCheck({
    ...
    successEvidence:
      `All ${claims.length} experience claims and all analysis hypotheses/checks resolve to measured or calculated evidence.`,
    recommendation:
      'Remove unsupported claims or attach them to a fact, calculation, hypothesis, unknown, or recommended check.'
  });

  const uncertaintyFailures: string[] = [];
  ...
  const surfacedUncertaintyRefs = new Set(
    experience.trustElements.uncertainty.sourceRefs
  );

  for (const claim of claims) {
    if (claim.claimType === 'hypothesis') {
      if (!claim.evidenceRefs.some(reference => hypothesisIds.has(reference))) {
        uncertaintyFailures.push(
          `Hypothesis claim "${claim.text}" does not reference a declared hypothesis.`
        );
      }
      if (!claim.uncertaintyNote) {
        uncertaintyFailures.push(
          `Hypothesis claim "${claim.text}" does not expose a limitation.`
        );
      }
    }
    ...
    if (claim.claimType === 'unknown') {
      if (!claim.evidenceRefs.some(reference => unknownIds.has(reference))) {
        uncertaintyFailures.push(
          `Unknown claim "${claim.text}" does not reference a declared unknown.`
        );
      }
      if (!claim.uncertaintyNote) {
        uncertaintyFailures.push(
          `Unknown claim "${claim.text}" does not state the knowledge boundary.`
        );
      }
    }

    if (claim.claimType === 'hypothesis' || claim.claimType === 'unknown') {
      const declaredRefs = claim.evidenceRefs.filter(
        reference => hypothesisIds.has(reference) || unknownIds.has(reference)
      );
      const missingTrustRefs = declaredRefs.filter(
        reference => !surfacedUncertaintyRefs.has(reference)
      );
      if (missingTrustRefs.length > 0) {
        uncertaintyFailures.push(
          `Trust elements do not surface uncertainty for: ${missingTrustRefs.join(', ')}.`
        );
      }
    }
  }
  ...
  const uncertainty = buildCheck({
    ...
    successEvidence:
      'Hypotheses and unknowns are labeled, qualified, and surfaced in the trust elements.',
    recommendation:
      'Label hypotheses and unknowns explicitly, add limitations, and surface them in the trust layer.',
    severity: 'medium'
  });
...
export const runExperienceAxCritic = (
  ...
  const missingVisibleRefs = [...priorityEvidenceRefs].filter(
    reference => !visibleEvidenceRefs.has(reference)
  );

  const evidenceVisibility = buildCheck({
    id: 'evidence_visibility',
    failures: missingVisibleRefs.map(
      reference => `Primary or explanatory claim evidence is not visible: ${reference}.`
    ),
    successEvidence:
      `All ${priorityEvidenceRefs.size} evidence references behind the primary message and explanation are visible.`,
    recommendation:
      'Expose the evidence behind the primary message and explanation before presenting the action.'
  });

  const progressiveDisclosureFailures: string[] = [];
  ...
  for (const expectedHiddenRef of [
    ...
    if (!hiddenRefs.has(expectedHiddenRef)) {
      progressiveDisclosureFailures.push(
        `Supporting load detail is not progressively disclosed: ${expectedHiddenRef}.`
      );
    }
  }
  ...
  const progressiveDisclosure = buildCheck({
    ...
    successEvidence:
      'One primary action and five essential evidence items are shown; supporting load and raw-reading detail is deferred.',
    recommendation:
      'Keep one primary action visible and defer supporting load or technical telemetry.',
    severity: 'medium'
  });

  const autonomyFailures: string[] = [];
  const safeModes = new Set<string>(SCENARIO_001_ALLOWED_AUTONOMY_MODES);
  const declaredModes = new Set<string>(scenario.allowedAutonomyModes);
  const axModes = new Set<string>(ax.allowedAutonomyModes);
  const trustModes = new Set<string>(
    experience.trustElements.autonomyBoundary.allowedModes
  );

  if (
    scenario.scenarioId !== analysis.scenarioId ||
  ...
  for (const capability of prohibitedCapabilities) {
    if (!ax.aiCannot.includes(capability as 'change_schedule' | 'control_equipment')) {
  ...
  const unsupportedVerificationRefs = ax.verification.evidenceRefs.filter(
    reference => !analysisEvidenceIds.has(reference)
  );
  if (unsupportedVerificationRefs.length > 0) {
    autonomyFailures.push(
      `AX verification has unsupported references: ${unsupportedVerificationRefs.join(', ')}.`
    );
  }
  ...
  const autonomySafety = buildCheck({
    ...
    successEvidence:
      'Scenario 001 stays within Assist/Investigate/Recommend and cannot change schedules or control equipment.',
    recommendation:
      'Restrict Scenario 001 to Assist, Investigate, and Recommend; keep all control actions human-owned.'
  });
