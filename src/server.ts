...
import { generatePerspectiveReport, PerspectiveContext, PerspectiveMode } from './llm.js';
...
import {
  ...
  listKnowledgeContextInputSchema,
  listMemoryContextInputSchema,
  generateCorrectionPromptInputSchema,
  storeKnowledgeContextInputSchema,
  storeMemoryContextInputSchema,
...
export class UxReviewServer {
  ...
  constructor(hasDebugFlag = false) {
    ...
    this.server = new Server(
      { name: 'ux-review-mcp', version: '0.1.0' },
      { capabilities: { tools: {} } }
    );
  ...
  private getDesignDataForRun(runId: number): ExtractedDesignData | undefined {
    ...
    const parsed = toolCalls
      ...
      .map(tc => {
        try {
          return { toolName: tc.tool_name, status: 'success' as const, data: JSON.parse(tc.raw_json!) };
        } catch {
          return null;
        }
      })
      .filter((item): item is { toolName: string; status: 'success'; data: unknown } => item !== null);

    if (parsed.length === 0) return undefined;
    return extractDesignData(parsed, 'figma');
  }

  private readOutputRefContent(outputRef: string): string | null {
  ...
  private async runReviewInputFromArgs(
    args: Record<string, unknown> | undefined,
    onProgress?: (stage: string, progress: number, total: number) => void
  ) {
    return reviewInput(
      {
        figmaUrl: args?.figmaUrl as string | undefined,
        webUrl: args?.webUrl as string | undefined,
        imagePath: args?.imagePath as string | undefined,
        htmlSnippet: args?.htmlSnippet as string | undefined,
        designSystemFigmaUrl: args?.designSystemFigmaUrl as string | undefined,
        chatContext: args?.chatContext as string | undefined,
        problemStatement: args?.problemStatement as string | undefined,
        proposedSolution: args?.proposedSolution as string | undefined,
        requirements: args?.requirements as string[] | undefined,
        designSystem: (args?.designSystem as string | undefined) ?? 'generic',
        customGuidelinePath: args?.customGuidelinePath as string | undefined
      },
      this.storage,
      this.hasDebugFlag,
      onProgress,
  ...
  private persistAnalysisContext(runId: number, input: any) {
    ...
    const storedKnowledge = this.storage.upsertKnowledgeItems(
    ...
    const storedMemories = this.storage.upsertMemoryEntries(
      memoryEntries.map((item: any) => ({
    ...
    this.storage.saveAnalysisMetadata({
      ...
      memoryKeys: memoryEntries.map((item: any) => `${item.memoryScope}:${item.memoryKey}:${item.entryType}`)
    });

    const detail = this.storage.getReportDetail(runId) as Record<string, unknown> | undefined;
    if (detail) {
      const mergedDetail = {
        ...detail,
        analysisContext: {
  ...
  private async runAnalysisFromArgs(
    args: Record<string, unknown> | undefined,
    onProgress?: (stage: string, progress: number, total: number) => void
  ) {
    const input = analyzeDesignInputSchema.parse(args ?? {}) as any;
  ...
  private getEnrichedContextForRun(runId: number) {
    const knowledgeContext = this.storage.getKnowledgeContextForRun(runId);
    if (!knowledgeContext) {
      return {};
    }

    const groupMemoryEntries = (scope: 'session' | 'user' | 'project' | 'organization') =>
      knowledgeContext.memoryEntries
  ...
  private async storeKnowledgeContext(args: Record<string, unknown> | undefined) {
    const input = storeKnowledgeContextInputSchema.parse(args ?? {}) as any;
    const knowledgeItems = this.storage.upsertKnowledgeItems(
      input.knowledgeItems.map((item: any) => ({
        knowledgeKey: typeof item.knowledgeKey === 'string' ? item.knowledgeKey : undefined,
        userId: typeof item.userId === 'string' ? item.userId : input.userId,
        projectId:
          typeof item.projectId === 'number'
  ...
  private async listKnowledgeContext(args: Record<string, unknown> | undefined) {
    const input = listKnowledgeContextInputSchema.parse(args ?? {});
    const preferredScopes: Array<'session' | 'user' | 'project' | 'organization'> = input.sessionId
  ...
  private async storeMemoryContext(args: Record<string, unknown> | undefined) {
    const input = storeMemoryContextInputSchema.parse(args ?? {}) as any;
    const memories = this.storage.upsertMemoryEntries(
      input.memories.map((item: any) => ({
        memoryScope: item.memoryScope,
        memoryKey: String(item.memoryKey ?? ''),
        entryType: String(item.entryType ?? ''),
        content: item.content,
  ...
  private async listMemoryContext(args: Record<string, unknown> | undefined) {
  ...
  private async buildPerspectiveFromRun(
    runId: number,
    ...
    const detail = this.storage.getReportDetail(runId) as any;
    if (!detail) {
    ...
    const designSystemFindings = (this.storage.getDesignSystemFindings(runId) as any) ?? {};
    const designData = this.getDesignDataForRun(runId);
    const extraContext = this.getEnrichedContextForRun(runId);

    const perspectiveCtx: PerspectiveContext = {
      mode,
      ...
      tools: [
        {
          name: 'review_figma',
          description:
            'Review a Figma design for UX quality. Fetches design data via Figma MCP, ' +
          ...
          inputSchema: {
            ...
            properties: {
              figmaUrl: {
                type: 'string',
                description:
                  'Full Figma URL (e.g. https://www.figma.com/design/KEY/name?node-id=X-Y)'
              },
              designSystem: {
                type: 'string',
                description:
                  "Design system mode: 'generic', 'external', 'custom', 'none' (default: 'generic')"
              },
              customGuidelinePath: {
                type: 'string',
                description:
                  'Path to custom guideline markdown file (required if designSystem=custom)'
              },
              problemStatement: {
                type: 'string',
                description:
                  'Optional explicit problem statement used for deterministic fit scoring'
              },
              proposedSolution: {
                type: 'string',
                description:
                  'Optional explicit proposed solution summary used for deterministic fit/traceability scoring'
              },
              requirements: {
                type: 'array',
                items: { type: 'string' },
                description:
                  'Optional list of explicit requirements used for deterministic traceability scoring'
              }
            },
            required: ['figmaUrl']
          }
        },
        {
          name: 'review_input',
          description:
            'Run UX review from multiple inputs: figmaUrl, webUrl, imagePath, htmlSnippet, or auto-detect from chatContext. ' +
          ...
          inputSchema: {
            ...
            properties: {
              ...
              chatContext: {
                type: 'string',
                description:
                  'Recent chat text used for auto-detect when explicit inputs are missing'
              },
              designSystem: {
                type: 'string',
                description:
        ...
        },
        {
          ...
          description:
            'Canonical design analysis entry point. Accepts the same review inputs plus optional user/project/session context, knowledge items, relationships, and memory entries. Returns markdown first, then JSON metadata.',
          inputSchema: {
            type: 'object' as const,
            properties: {
              ...
              designSystemFigmaUrl: {
                type: 'string',
                description: 'Optional Figma design system URL used as explicit review context'
              },
        ...
        },
        {
          ...
          description:
            'Retrieve structured report detail JSON for side-panel rendering and score indicators.',
          inputSchema: {
            type: 'object' as const,
            properties: {
              runId: {
        ...
        },
        {
          name: 'list_reviews',
          description:
            'List recent review runs with stage and status metadata for desktop history/panel navigation.',
          inputSchema: {
            type: 'object' as const,
            properties: {
              limit: {
        ...
        },
        {
          ...
          description:
            'Store categorized UX/domain/product knowledge and lightweight relationships for a user, project, or session.',
          inputSchema: {
            type: 'object' as const,
            properties: {
              userId: { type: 'string' },
        ...
        },
        {
          ...
          description:
            'List knowledge items and relationships for a user/project/session with optional ranked retrieval (priority + tags + scope).',
          inputSchema: {
            type: 'object' as const,
            properties: {
              userId: { type: 'string' },
        ...
        },
        {
          ...
          description:
            'Store session, user, or project memory entries that should inform future analysis runs.',
          inputSchema: {
            type: 'object' as const,
            properties: {
              memories: {
        ...
        },
        {
          ...
          description:
            'List stored memory entries to help inspect what the assistant has learned for a user or project.',
          inputSchema: {
            type: 'object' as const,
            properties: {
              memoryScope: {
        ...
        },
        {
          ...
          description:
            'Challenge the current design from problem-solution-requirement perspective and highlight missed edge cases.',
          inputSchema: {
            type: 'object' as const,
            properties: {
              runId: {
        ...
        },
        {
          ...
          description:
            'Run review_input and challenge_design in one call. Accepts same input fields as review_input plus optional prdText.',
          inputSchema: {
            type: 'object' as const,
            properties: {
              ...
              designSystemFigmaUrl: { type: 'string' },
        ...
        },
        {
          name: 'improve_design',
          description:
            'Provide concrete design improvements (visual, interaction, component selection, edge cases).',
          inputSchema: {
            type: 'object' as const,
            properties: {
              runId: {
              ...
              requirements: {
                ...
                items: { type: 'string' },
                description:
                  'Optional requirements list to validate proposed improvements against'
              }
            },
            required: ['runId']
          }
        },
        {
          ...
          description:
            'Run review_input and improve_design in one call. Accepts same input fields as review_input.',
          inputSchema: {
            type: 'object' as const,
            properties: {
              figmaUrl: { type: 'string' },
        ...
        },
        {
          name: 'pitch_design',
          description:
            'Generate stakeholder-facing pitch points and decision rationale from review findings.',
          inputSchema: {
            type: 'object' as const,
            properties: {
              runId: {
              ...
              audience: {
                type: 'string',
                description:
                  'Optional stakeholder audience (e.g., PM, leadership, engineering, design review board)'
              },
              businessGoal: {
                type: 'string',
              ...
              designDecisions: {
                ...
                items: { type: 'string' },
                description:
                  'Designer\'s own rationale for key decisions (e.g., "We chose tabs over accordion because users need to compare sections")'
              },
              constraints: {
                type: 'array',
                items: { type: 'string' },
                description:
                  'Technical/business constraints that shaped the design (e.g., "Must work offline", "Legacy API limitation")'
              },
              alternativesConsidered: {
                type: 'array',
                items: { type: 'string' },
                description:
                  'Alternatives the designer explored and rejected (e.g., "Tried wizard flow but users abandoned at step 3")'
              },
              userResearch: {
                type: 'string',
                description:
                  'User research or data backing the design approach (interview quotes, analytics, usability test findings)'
              }
            },
            required: ['runId']
          }
          ...
          inputSchema: {
            ...
            properties: {
              ...
              designDecisions: {
                ...
                items: { type: 'string' },
                description:
                  'Designer\'s own rationale for key decisions'
              },
              constraints: {
                type: 'array',
                items: { type: 'string' },
                description:
                  'Technical/business constraints that shaped the design'
              },
              alternativesConsidered: {
                type: 'array',
                items: { type: 'string' },
                description:
                  'Alternatives the designer explored and rejected'
              },
              userResearch: {
                type: 'string',
                description:
                  'User research or data backing the design approach'
              }
            },
            required: ['runId']
          }
        },
        {
          ...
          description:
            'Compare two review runs to show progress, regression, and what changed between design iterations.',
          inputSchema: {
            type: 'object' as const,
            properties: {
              currentRunId: {
        ...
        },
        {
          name: 'gather_context',
          description:
            'Gather additional design context from other available MCP tools via sampling. ' +
        ...
        },
        {
          ...
          description:
            'Create a context pack from selected rule IDs. If omitted, approved rules in the project are used.',
          inputSchema: {
            type: 'object' as const,
            properties: {
              projectId: { type: 'number' },
        ...
        },
        {
          ...
          description:
            'Validate an output against a context pack. Uses deterministic checks where available and unknown findings where not yet implemented.',
          inputSchema: {
            type: 'object' as const,
            properties: {
        ...
        },
        {
          ...
          description:
            'Generate a tool-specific correction prompt from validation findings and store it in prompt history.',
          inputSchema: {
            type: 'object' as const,
            properties: {
        ...
        },
        {
          ...
          description:
            'Compare two validation runs to show compliance progress, regressions, and rule-level status changes.',
          inputSchema: {
            type: 'object' as const,
            properties: {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
      ...
      switch (name) {
        ...
        case 'reviewfigma': {
          ...
          try {
            const onProgress = sendProgress
              ? (_stage: string, progress: number, total: number) => { sendProgress(progress, total, _stage); }
              : undefined;
              ...
              content: [
                {
                  type: 'text',
                  text:
                    (report ?? 'Report not available for this run.') +
                    '\n\n---\n\n' +
                    '## Run Metadata (JSON)\n\n' +
                    '```json\n' +
                    JSON.stringify(
        ...
        case 'get_review_report': {
          const runId = args?.runId as number | undefined;
          if (!runId) {
        ...
        case 'get_review_status': {
          ...
          const run = this.storage.getRun(runId) as
            | {
                id: number;
                status: string;
                stage: string;
        ...
        case 'challengedesign': {
          ...
          const missingExplicitContext =
            !prdText && (!strategicBranch || strategicBranch?.inputSource === 'derived-proxy');
          ...
          const edgeCaseFindings = Array.isArray(strategic?.edgeCaseFindings)
            ...
            : [];
          const highFindings = edgeCaseFindings.filter(
            (item: any) => String(item?.severity ?? '').toLowerCase() === 'high'
          );
          const medFindings = edgeCaseFindings.filter(
            (item: any) => String(item?.severity ?? '').toLowerCase() === 'medium'
          );
          const lowFindings = edgeCaseFindings.filter(
            (item: any) => String(item?.severity ?? '').toLowerCase() === 'low'
          );
          ...
          const getSection = (parameter: string) =>
            sections.find((section: any) => section?.parameter === parameter) ?? null;

          const severityFromPct = (pct: unknown): 'HIGH' | 'MEDIUM' | 'LOW' => {
          ...
          const severityFromIssues = (section: any): 'HIGH' | 'MEDIUM' | 'LOW' => {
            ...
            if (
              issues.some((item: any) => String(item?.severity ?? '').toLowerCase() === 'medium')
            ) {
              return 'MEDIUM';
            }
          ...
          const summarizeSection = (section: any, fallback: string): string[] => {
            if (!section) return [fallback];

            const notes: string[] = [];
            if (section.summary) notes.push(section.summary);
            if (section.narrative && section.narrative !== section.summary) notes.push(section.narrative);

            const issues = Array.isArray(section.issues) ? section.issues.slice(0, 2) : [];
            for (const issue of issues) {
              notes.push(
                `${issue.title}: ${issue.evidence} Recommendation: ${issue.recommendation}`
              );
            }
          ...
          const parsePrdContext = (value: string) => {
            const parsed: { problem: string; solution: string; requirements: string[] } = {
              problem: '',
              solution: '',
              requirements: []
            };
            ...
            for (const rawLine of linesInPrd) {
              ...
              if (/^requirements\s*:/i.test(line)) {
                ...
                if (inline) parsed.requirements.push(inline);
                continue;
              }
          ...
          const inferRequirementParameter = (requirement: string) => {
            const text = String(requirement ?? '').toLowerCase();
            if (/(wcag|accessib|keyboard|contrast|screen reader)/.test(text)) return 'accessibility_wcag';
            if (/(brand|visual|layout|hierarchy|spacing|color|typography)/.test(text)) return 'visual_hierarchy_layout';
            if (/(component|token|design system|icon)/.test(text)) return 'design_system_consistency';
            if (/(search|find|filter|chat|navigation|browse|flow|group|pin|tag)/.test(text)) return 'user_flow_interaction';
            if (/(label|content|information|copy|taxonomy|ia|architecture)/.test(text)) return 'content_information_architecture';
            if (/(performance|technical|latency|render|implementation)/.test(text)) return 'technical_feasibility';
            return 'user_flow_interaction';
          };

          const impactForParameter = (parameter: string) => {
            switch (parameter) {
          ...
          const requirementsList =
            parsedContext.requirements.length > 0
          ...
          lines.push(
            `- Requirement Traceability: ${strategicBranch?.requirementTraceabilityPct ?? 'n/a'}%`
          );
          ...
          if (missingExplicitContext) {
            ...
            lines.push(
              'The challenge quality is currently limited because explicit problem/requirements context is missing.'
            );
            lines.push('');
          ...
          lines.push(
            `Problem Statement: ${parsedContext.problem || 'No explicit problem statement was provided beyond the run context.'}`
          );
          for (const note of summarizeSection(
            problemSection,
            'The current review does not yet prove that the primary find-chat task is measurably faster.'
          )) {
            lines.push(`- ${note}`);
          }
          ...
          lines.push(
            `Proposed Solution: ${parsedContext.solution || 'Grouping chats into a project-based container is the stated solution.'}`
          );
          for (const note of summarizeSection(
            solutionSection,
            'The solution introduces structure, but the review needs stronger evidence that the grouping model matches user mental models.'
          )) {
            lines.push(`- ${note}`);
          }
          ...
          if (requirementsList.length === 0) {
            lines.push(
              '| No explicit requirements provided | Review cannot trace coverage | Needs input | Provide requirements in `prdText` for better traceability. |'
            );
          } else {
            for (const requirement of requirementsList) {
              ...
              lines.push(
                `| ${String(requirement).replace(/\|/g, '/')} | ${toTitle(parameter)} ${typeof section?.alignmentPct === 'number' ? `(${section.alignmentPct}%)` : ''} | ${status} | ${String(notes).replace(/\|/g, '/')} |`
              );
            }
          }
          ...
          for (const note of summarizeSection(
            iaSection,
            'Navigation structure needs stronger validation for findability and recovery paths.'
          )) {
            lines.push(`- ${note}`);
          }
          ...
          for (const note of summarizeSection(
            visualSection,
            'The visual system does not yet prove that the most important chat actions stand out immediately.'
          )) {
            lines.push(`- ${note}`);
          }
          ...
          if (designSystemComponents.length === 0 && designSystemIcons.length === 0) {
            lines.push(
              '| No explicit design-system evidence captured | n/a | n/a | No pass/fail assessment could be derived from this run. |'
            );
          } else {
            for (const component of designSystemComponents.slice(0, 5)) {
              const assessment =
                component.matchScore >= 0.75
                  ? 'Pass'
                  : component.matchScore >= 0.5
                    ? 'Partial'
                    : 'Fail';
              lines.push(
                `| ${component.componentName} | ${Math.round(component.matchScore * 100)}% | ${component.category} | ${assessment}: ${String(component.description ?? '').replace(/\|/g, '/')} |`
              );
            }
            for (const icon of designSystemIcons.slice(0, 3)) {
              lines.push(
                `| ${icon.iconName} | assumed | icon | Partial: icon evidence was inferred but not strongly verified in this run. |`
              );
            }
          }
          ...
          if (edgeCaseFindings.length === 0) {
            lines.push(
              '| No explicit edge-case findings captured | Review depth is limited by available evidence | Medium | Medium | Add more explicit task, accessibility, and navigation evidence. |'
            );
          } else {
              ...
              const likelihood =
                String(item.severity ?? '').toLowerCase() === 'high'
                  ...
                  : String(item.severity ?? '').toLowerCase() === 'medium'
                    ...
                    : 'Low';
              lines.push(
                `| ${toTitle(item.parameter)} | ${String(item.evidence ?? 'No evidence provided.').replace(/\|/g, '/')} | ${likelihood} | ${impactForParameter(item.parameter)} | ${String(item.recommendation ?? '').replace(/\|/g, '/')} |`
              );
            }
          }
          ...
          if (
            ...
            lines.push(
              'Verdict: HIGH RISK. The current design direction may help organize chats, but this review does not yet prove that grouping is the fastest path for finding the right chat.'
            );
          } else if (
            ...
            lines.push(
              'Verdict: MEDIUM RISK. The design has a credible direction, but key assumptions still need clearer evidence and requirement traceability.'
            );
          } else {
            lines.push(
              'Verdict: LOW RISK. The design direction appears aligned, with only limited follow-up validation needed.'
            );
          }
          ...
          if (strategicBranch?.branch === 'persuasion') {
            lines.push(
              '1. Lead stakeholder review with the strongest observed proof points and explicitly address the highest-risk objections.'
            );
            lines.push(
              '2. Convert remaining high-severity findings into mitigation commitments with owners and timing.'
            );
            lines.push(
              '3. Re-run the review after stakeholder feedback to confirm objections were actually resolved.'
            );
          } else if (strategicBranch?.branch === 'improvement') {
            lines.push(
              '1. Fix the highest-severity flow and accessibility gaps before polishing secondary screens.'
            );
            lines.push(
              '2. Prototype lighter alternatives such as search, pins, or auto-grouping and compare against the project-grouping concept.'
            );
            lines.push(
              '3. Validate success with a time-to-chat task, not just visual preference feedback.'
            );
          } else {
            lines.push(
              '1. Treat this as a dual-track review: improve the core findability flow while also preparing a stakeholder-ready rationale.'
            );
            lines.push(
              '2. Compare the grouping concept against lighter alternatives such as search, pins, recents, or auto-tagging.'
            );
            lines.push(
              '3. Bring back measurable evidence on task speed, navigation confidence, and accessibility compliance before final sign-off.'
            );
          }
          ...
          if (highFindings.length > 0) {
            lines.push(
              '1. Resolve all HIGH-severity findings first, starting with accessibility and flow breakpoints.'
            );
          } else {
          ...
          if (prdText) {
            ...
            lines.push('');
            lines.push(
              'Use the prompts above to validate whether PRD problem framing, solution strategy, and requirement traceability are complete.'
            );
          }
        ...
        case 'improvedesign': {
          ...
          const problemStatement =
            typeof args?.problemStatement === 'string' ? args.problemStatement.trim() : '';
          const requirements = Array.isArray(args?.requirements)
            ? args.requirements.filter(
          ...
          lines.push(
            '2. Run edge-case checks for loading, error, empty, and keyboard-only interaction states.'
          );
          lines.push(
            '3. Execute next experiments and compare pre/post alignment scores before finalizing.'
          );
          return { content: [{ type: 'text', text: lines.join('\n') }] };
        }
        ...
        case 'improve_from_input': {
          try {
            ...
            const review = await this.runReviewInputFromArgs(args, onProgress);
            const problemStatement =
              typeof args?.problemStatement === 'string' ? args.problemStatement.trim() : undefined;
            const requirements = Array.isArray(args?.requirements)
              ? args.requirements.filter(
                  (item): item is string => typeof item === 'string' && item.trim().length > 0
                )
        ...
        case 'pitchdesign': {
          ...
          const businessGoal =
            typeof args?.businessGoal === 'string' ? args.businessGoal.trim() : '';
          const designDecisions = Array.isArray(args?.designDecisions)
            ? args.designDecisions.filter((item): item is string => typeof item === 'string' && item.trim().length > 0)
            : [];
          ...
          const alternativesConsidered = Array.isArray(args?.alternativesConsidered)
            ? args.alternativesConsidered.filter((item): item is string => typeof item === 'string' && item.trim().length > 0)
            : [];
          ...
          // Fallback to deterministic template if LLM unavailable
          const strategicBranch = detail?.strategicBranch ?? null;
          const missingPitchContext =
            !audience && !businessGoal && (!strategicBranch || strategicBranch?.inputSource === 'derived-proxy');
          ...
          lines.push(
            pack?.convincePartners ?? 'No stakeholder persuasion guidance available for this run.'
          );
          ...
          lines.push('## Prioritized Action Plan');
          lines.push(
            '1. Lead with business outcome and top 2 value points tied to measurable impact.'
          );
          lines.push('2. Use proof points to preempt highest-risk objections before open Q&A.');
          lines.push(
            '3. Close with implementation next steps, owners, and timeline for high-severity gaps.'
          );
          return { content: [{ type: 'text', text: lines.join('\n') }] };
        }
        ...
        case 'list_context_rules': {
          ...
          const status = args?.status as 'draft' | 'approved' | 'deprecated' | undefined;
        ...
        case 'approve_context_rule': {
          const rulePkId = args?.rulePkId as number | undefined;
          if (!rulePkId) {
        ...
        case 'validate_output_against_context': {
          try {
            ...
            const findings = packRules.map(rule =>
              evaluateRuleFinding(
                {
                  ruleId: rule.ruleId,
        ...
        case 'generate_correction_prompt': {
          try {
            ...
            const actionableCount = findings.filter(
              item => item.status === 'fail' || item.status === 'partial'
            ).length;
        ...
        case 'compare_validation_runs': {
          try {
            ...
            const toRank = (status: string): number => {
            ...
            for (const finding of currentFindings) {
              ...
              if (currentRank > previousRank) improvedRules += 1;
              else if (currentRank < previousRank) regressedRules += 1;
              else unchangedRules += 1;
            }
        ...
        case 'compare_reviews': {
          const currentRunId = args?.currentRunId as number | undefined;
          const previousRunId = args?.previousRunId as number | undefined;
          if (!currentRunId || !previousRunId) {
            return { content: [{ type: 'text', text: 'Error: both currentRunId and previousRunId are required' }] };
          }

          const currentDetail = this.storage.getReportDetail(currentRunId) as any;
          const previousDetail = this.storage.getReportDetail(previousRunId) as any;
        ...
        case 'gather_context': {
          const designRef = args?.designRef as string | undefined;
          if (!designRef) {
            return { content: [{ type: 'text', text: 'Error: designRef is required' }] };
          }

          const contextTypes = (Array.isArray(args?.contextTypes) ? args.contextTypes : ['accessibility', 'design-system', 'user-flows', 'content', 'technical']) as Array<'accessibility' | 'design-system' | 'user-flows' | 'content' | 'technical'>;
