...
export const evaluateCodeLikeRule = (
  ...
  if (text.includes('cta') || text.includes('primary')) {
    ...
    return {
      ...
      correctionPrompt:
        'Revise the page so only one primary CTA remains in the hero/top section. Convert other competing actions to secondary or text links.'
    };
  }
  ...
  if (text.includes('token') || text.includes('design system') || text.includes('color')) {
    ...
    return {
      ...
      correctionPrompt:
        'Replace hardcoded color literals with approved design-system tokens and ensure components use the canonical theme variables.'
    };
  }
