import { experienceRecommendationSchema } from './schemas.js';
const findEquipmentCalculation = (analysis, equipmentId) => {
    const calculation = analysis.calculations.equipmentContributions.find(item => item.equipmentId === equipmentId);
    if (!calculation) {
        throw new Error(`Missing equipment contribution for ${equipmentId}.`);
    }
    return calculation;
};
export const buildExperienceRecommendation = (analysis) => {
    ...
    return experienceRecommendationSchema.parse({
        ...
        primaryMessage: {
            text: `Energy use increased ${total.percentChange}%. ` +
        ...
        explanation: {
            summary: {
                text: 'HVAC is the dominant measured contributor, and its longer operating time warrants investigation.',
                claimType: 'calculation',
                evidenceRefs: [hvac.id, hours.id]
            },
            claims: [
                {
                    text: `HVAC energy rose ${hvac.percentChange}% and operating time increased ` +
                        `${hours.deltaHoursPerDay} hours per day.`,
                    claimType: 'calculation',
                    evidenceRefs: [hvac.id, hours.id]
                },
                {
                    text: 'The longer operating time may be contributing to the HVAC increase, but it does not establish the cause.',
        ...
        evidence: [
            ...
            },
            {
                ...
                value: `+${hvac.percentChange}% energy; ` +
        ...
        trustElements: {
            ...
            uncertainty: {
                text: 'Extended HVAC operation is a working hypothesis; the reason for the schedule change is still unknown.',
