...
const remoteFigmaToolSource = readFileSync(join(process.cwd(), 'src', 'tools', 'review-figma.ts'), 'utf8');
const remoteInputToolSource = readFileSync(join(process.cwd(), 'src', 'tools', 'review-input.ts'), 'utf8');
const remoteChallengeToolSource = readFileSync(join(process.cwd(), 'src', 'tools', 'challenge-design.ts'), 'utf8');
const remoteImproveToolSource = readFileSync(join(process.cwd(), 'src', 'tools', 'improve-design.ts'), 'utf8');
const remotePitchToolSource = readFileSync(join(process.cwd(), 'src', 'tools', 'pitch-design.ts'), 'utf8');
...
test('remote follow-up tools synthesize from existing review runs', () => {
    ...
    assert.doesNotMatch(`${remoteChallengeToolSource}\n${remoteImproveToolSource}\n${remotePitchToolSource}`, /Phase 1|registered and available remotely|serverless-safe phase 2 migration|Perspective generation will be wired/);
});
