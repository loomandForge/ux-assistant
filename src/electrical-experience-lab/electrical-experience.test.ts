...
import {
  ...
  scenario001AxRecommendationSchema
} from './schemas.js';
...
test('Scenario 001 flows through deterministic analysis, recommendations, AX, and critics', () => {
  ...
  const equipmentCalculations = Object.fromEntries(
    result.analysis.calculations.equipmentContributions.map(item => [
      item.equipmentId,
      item
    ])
  );
  ...
  assert.equal(
    result.analysis.unknowns[0]?.statement,
    'The reason for the HVAC schedule change is unknown.'
  );
  ...
  assert.deepEqual(
    new Set(result.critics.results.flatMap(critic => critic.checks.map(check => check.id))),
    new Set([
      'unsupported_claims',
      'evidence_visibility',
