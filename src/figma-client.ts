 ...
 */
export async function fetchFigmaData(options: FigmaFetchOptions): Promise<ToolCallResult[]> {
  const { fileKey, nodeId, figmaUrl, debug } = options;
  const results: ToolCallResult[] = [];

  const client = new Client(
    { name: 'ux-review-figma-client', version: '0.1.0' },
    { capabilities: {} }
  );
  ...
  try {
    ...
    if (debug) {
      console.error(
        `[Figma MCP] Connected. Available tools: ${[...availableToolNames].join(', ')}`
      );
    }
    ...
    for (const toolName of FIGMA_TOOLS_TO_CALL) {
      ...
      try {
        const args: Record<string, string> = { figmaUrl };
        ...
        if (debug) console.error(`[Figma MCP] ${toolName} failed: ${msg}`);
      }
    }
  } catch (error) {
    const msg = error instanceof Error ? error.message : String(error);
