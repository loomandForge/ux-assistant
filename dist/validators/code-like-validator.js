...
const hasPrimaryClassToken = (value) => {
    if (!value)
        return false;
    return value
        .split(/\s+/)
        .some(token => /^(primary|primary-cta|cta-primary|btn-primary|button-primary)$/i.test(token));
};
const hasPrimaryMarker = (element) => {
    ...
    return (hasPrimaryClassToken(className) ||
...
const elementLabel = (element) => {
    const explicit = getAttr(element.attrs, 'aria-label') ??
...
const extractHardcodedColors = (source) => unique([
    ...(source.match(/#[0-9a-fA-F]{3,8}\b/g) ?? []),
    ...(source.match(/\brgba?\([^)]+\)/gi) ?? []),
    ...(source.match(/\bhsla?\([^)]+\)/gi) ?? [])
]);
const extractHeadingLevels = (source) => [...source.matchAll(/<h([1-6])\b/gi)]
    .map(match => Number(match[1]))
    .filter(level => Number.isInteger(level));
const hasMainLandmark = (source) => /<main\b/i.test(source) || /role\s*=\s*["']main["']/i.test(source);
const hasNavigationLandmark = (source) => /<nav\b/i.test(source) || /role\s*=\s*["']navigation["']/i.test(source);
const hasFormControl = (source) => /<(form|input|textarea|select)\b/i.test(source);
const hasFieldLabel = (source) => /<label\b/i.test(source) || /aria-label\s*=|aria-labelledby\s*=/i.test(source);
const hasErrorStateEvidence = (source) => /aria-invalid\s*=\s*["']?true/i.test(source) ||
...
const evaluateCtaRule = (rule, outputContent, severity) => {
    ...
    if (count === 0) {
        return {
            ...
            correctionPrompt: 'Identify the one action users should take next and style it as the sole primary CTA.'
        };
    }
    return {
        ruleId: rule.ruleId,
        ...
        correctionPrompt: 'Keep one dominant primary CTA. Convert competing actions to secondary buttons, tertiary links, or progressive disclosure.'
    };
};
const evaluateColorRule = (rule, outputContent, severity) => {
    ...
    return {
        ...
        correctionPrompt: 'Replace each hardcoded color literal with the nearest approved semantic token, then re-run validation.'
    };
};
const evaluateHeadingRule = (rule, outputContent, severity) => {
    ...
    if (levels.length === 0) {
        return {
            ...
            correctionPrompt: 'Add one screen-level h1 and organize sections with sequential h2/h3 headings.'
        };
    }
    if (h1Count !== 1 || skipped) {
        return {
            ...
            recommendation: 'Use exactly one h1 and avoid skipped heading levels so the information hierarchy is explicit.',
            correctionPrompt: 'Normalize heading levels to one h1 followed by sequential section headings without skipped levels.'
        };
    }
    return {
        ruleId: rule.ruleId,
...
const evaluateSemanticRule = (rule, outputContent, severity) => {
    const missing = [];
    if (!hasMainLandmark(outputContent))
        missing.push('main landmark');
    if (!hasNavigationLandmark(outputContent))
        missing.push('navigation landmark');
    if (extractHeadingLevels(outputContent).length === 0)
    ...
    return {
        ...
        correctionPrompt: 'Wrap primary content in main, expose navigation with nav or role navigation, and add meaningful headings.'
    };
};
const evaluateFormErrorRule = (rule, outputContent, severity) => {
    ...
    if (hasErrorState) {
        return {
            ...
            correctionPrompt: 'Add label, aria-label, or aria-labelledby for each form control while preserving error-state hooks.'
        };
    }
    return {
        ruleId: rule.ruleId,
        ...
        correctionPrompt: 'Add aria-invalid, aria-describedby, and visible error messaging for invalid inputs; expose urgent errors with role alert when appropriate.'
    };
};
